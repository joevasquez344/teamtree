...
const Message = ({
  ...
  const miniTablet = window.screen.width > 768;

  return (
    <div
      key={message.id}
      onMouseOver={
        window.screen.width > 768 ? () => showMore(message.id) : null
      }
      onMouseLeave={window.screen.width > 768 ? hideMore : null}
      className={`group md:hover:bg-gray-900 ${
        message?.type === "task" && " "
      }  ${
        inputEl === message.id && "bg-gray-900"
      } transition ease-in-out w-[80%] md:w-[100%] duration-200 px-3 md:px-5 py-1 md:py-3`}
    >
                ...
                <>
                  ...
                  {message?.type === "task" ? (
                    <div className="border relative flex justify-between border-gray-600 group-hover:border group-hover:border-gray-700 my-2 w-full  p-3 rounded-md transition ease-in-out duration-200">
                      <div className="text-sm md:text-base whitespace-break-spaces">
                        {message.text}
                      </div>
